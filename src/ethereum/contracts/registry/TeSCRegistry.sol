...
interface TeSCRegistry {
    ...
    event RegistryChanged(bytes32 domain, address indexed contractAddr, EventType eventType);
    ...
    function add(bytes32 _domain, address _contractAddr) external;
    ...
    // @param _contractAddr The address of the endorsed contract
    function remove(bytes32 _domain, address _contractAddr) external;
    ...
    function getContractsFromDomain(bytes32 _domain) external view returns (address[] memory);
